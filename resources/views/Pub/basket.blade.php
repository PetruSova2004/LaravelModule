                        ...
                        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 row-cols-xl-4">
                            @foreach($products as $product)
                                @foreach($baskets as $basket)
                                    @if($basket->product_id == $product->id && $basket->user_id == $user->id)
                                        <div class="col">
                                            <div class="card rounded-0 product-card">
                                                <div class="card-header bg-transparent border-bottom-0">
                                                    <div class="d-flex align-items-center justify-content-end gap-3">
                                                        ...
                                                        <form method="post"
                                                              action={{route('basket-products.delete', ['id' => $product->id])}}>
                                                            @csrf
                                                            @method('DELETE')
                                                            <a href={{route('basket-products.delete', ['id' => $product->id])}}>
                                                                <button type="submit" class="cart-product-cancel"><i
                                                                        class='bx bx-trash'></i>
                                                                </button>
                                                            </a>
                                                        </form>
                                                        ...
                                                        @foreach($subcategories as $subcategory)
                                                            <a href={{route('sub-categories.read', ['subCategory' => $subcategory->id])}}>
                                                                @if($subcategory->id == $product->subcategory_id)
                                                                    ...
                                                                    <p class="product-category font-13 mb-1">{{$subcategory->title}}</p>
                                                                @endif
                                                                @endforeach
                                                            ...
                                                            <div class="d-flex align-items-center">
                                                                <div class="mb-1 product-price">
                                                                    @if(session('EUR'))
                                                                        @if($user->signed === 1)
                                                                            <span
                                                                                class="me-1 text-decoration-line-through">€{{ceil(\App\Modules\Pub\Currency\Services\CurrencyService::exchange('EUR', 'USD', $product->price)->result)}}
                                                                        </span>
                                                                            <span
                                                                                class="text-white fs-5">€{{ceil(\App\Modules\Pub\Currency\Services\CurrencyService::exchange('EUR', 'USD', $product->price)->result -(\App\Modules\Pub\Currency\Services\CurrencyService::exchange('EUR', 'USD', $product->price)->result / 100 * 15))}}
                                                                        </span>
                                                                        @endif
                                                                    @endif
                                                                        ...
                                                                        @if(session('USD'))
                                                                            @if($user->signed === 1)
                                                                                <span
                                                                                    class="me-1 text-decoration-line-through">${{ceil($product->price)}}
                                                                        </span>
                                                                                <span
                                                                                    class="text-white fs-5">${{ceil($product->price - ($product->price / 100 * 15))}}
                                                                        </span>
                                                                            @endif
                                                                        @endif
                                                                    ...
                                                                    @if($user->signed !== 1)
                                                                        <span
                                                                            class="text-white fs-5">${{$product->price}}
                                                                            </span>
                                                                    @endif
